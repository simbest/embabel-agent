...
- 模块状态为 Experimental，且无 Spring Boot 自动装配、无 starter——`embabel-agent-skills` 是纯手动接线的独立库，全仓库 main 代码无其他模块引用它。
- 脚本引擎默认为 `NoOpExecutionEngine`（拒绝一切执行），必须显式配置 Process 或 Docker 引擎才能执行脚本；上游文档“脚本执行不支持”的限制声明已过时，以代码为准。
- `allowed-tools` 字段仅解析不强制，不构成安全边界（见 9.9 节）。
- `Skills` 不可变、激活状态仅存内存——天然适配 9.8 节快照模式，但跨请求状态管理需平台实现。
- GitHub/Git URL 加载在运行时执行 `git clone`，纯内网环境不可用，需走 9.5 节平台物化路径（见 14.7 节）。

生命周期支撑度：创建（格式与校验原生，工具链自建）、注册（本地物化 API 原生，注册中心自建）、审批与版本（纯自建）、激活与使用（原生）。当前缺少企业级注册中心、版本库、审批、灰度、回滚、租户权限和热发布管理。
...
官方 starter 已覆盖主流模型，其中 Z.ai（GLM 系列）与 DeepSeek 为 Stable 状态，MiniMax 有官方接入文档，DashScope（通义千问）1.5 起提供官方 starter；`embabel-agent-starter-byok`（1.5 新增，Incubating）支持密钥运行时供给、无密钥也能启动，适配企业密钥治理（1.5.1 开发线增 Atlas Cloud 支持）。
...
| embabel-agent-starter-dashscope | 1.5 新增 | 阿里云 DashScope（通义千问），代码已入库（官方 modules 目录暂未列出） |
| embabel-agent-starter-byok | Incubating | 密钥运行时供给，无密钥启动；1.5.1 增 Atlas Cloud |
...
- 系统提示可经 `TemplatedPromptRunnerBuilder`（1.5 新增）从模板渲染，便于人设与提示词的集中管理。
...
### 10.8 开源 MCP Server/CLI 制品接入治理

企业接入 GitHub 开源 Server/CLI（典型如 iOfficeAI/OfficeCLI：自包含单二进制 Office 文档工具，内置 `officecli mcp` stdio Server，另按 Agent Skills 规范分发 SKILL.md）时，注册模型（10.2 节）已可表达其形态（`transport=stdio`、`command=officecli`），但治理必须把第 9 章对 Skill 制品的供应链要求延伸到 Server 制品：

- 制品来源与分发：二进制/镜像经内网制品库下发，记录来源仓库与 commit；禁止生产环境运行时从公网安装（brew/scoop/npm 通道不进生产）。
- 完整性校验：SHA-256（有签名则验签）入库与部署两次校验，同 9.4 节。
- 版本固定：关闭制品自更新（如 `officecli config autoUpdate false`），版本变更走 9.3 节审批状态机——MCP 接入审批直接复用该状态机，不另建流程。
- 出站网络策略：Server 进程默认无出站，按白名单放行（自更新、遥测、watch 类外联默认拒绝）。
- 本地进程沙箱：`transport=stdio` 的 Server 与宿主 JVM 同机运行，须复用 9.9 节沙箱要求（资源限额、只读挂载、环境变量不继承）。
- 纯 CLI 直调形态：不经 MCP、由 Bash 工具或自写 `@LlmTool`（`ProcessBuilder`）直调 CLI 的，纳入同一注册模型，命令白名单按子命令粒度配置；注意 `BashTools` 硬编码 `/bin/bash -c`，Linux 节点原生可用，Windows 节点建议自写工具类。

接入配置示例（Spring AI 标准客户端配置，工具经 `McpToolGroup` 懒加载，可按谓词裁剪工具面）：
...
spring.ai.mcp.client:
  ...
  stdio:
    connections:
      officecli:
        command: officecli   # 内网制品库下发后以绝对路径指向固定版本二进制
        args: [mcp]
```
...
RAG 为 agentic 模式：LLM 自主决定检索时机与查询，ToolishRag 按存储能力渐进暴露工具。1.5 新增 `SectionReader`（按章节名导航文档）与命中邻近 chunk 获取，对合同等长结构化文档的条款级检索直接受用。同版本另有三项检索行为变化：全文检索得分绝对化、取消由 LLM 猜测相似度阈值、chunk 结构元数据升级为类型化 ChunkStructure。企业知识库选型建议：起步用 Lucene，生产用 pgvector（与业务库同引擎，运维收敛）。
...
版本基线注意：1.5.0 起框架自身已完成 Spring Boot 4 / Spring AI 2 GA（Jackson 3）迁移；嵌入式应用的 Spring Boot 大版本必须与所引入 starter 的版本线对齐，JDK 基线 17+。
...
| 存量老应用（Spring Boot 2 / JDK 8） | 无法嵌入（JDK 基线 17+，Spring Boot 大版本须与 starter 版本线对齐，1.5.0 起为 SB4），只能经平台应用 REST 调用 |

标准演进路径：先嵌入式跑通业务，公共 Agent 沉淀到平台应用，再经 MCP/A2A 回供各系统。本章 K8s 拓扑中的 Agent Runtime Pods 即上述应用（无论嵌入式或平台形态）的运行载体。

### 14.7 纯内网（无互联网）部署

框架运行期无强制外联，纯内网 Linux 服务器部署可行，但以下前提必须逐项落实：

| 依赖 | 内网前提 |
|---|---|
| LLM 推理 | Ollama/Docker Models starter 本地推理，或 deepseek/zai/openai-custom starter 指向内网 vLLM 等 OpenAI 兼容端点；云端 starter 不部署 |
| Skill 分发 | GitHub/Git URL 运行时拉取不可用：技能制品托管企业内网 Git，平台按 9.5 节物化到本地目录后加载 |
| 脚本沙箱 | Docker 引擎镜像预置内网 registry；Process 引擎所需 python/node 等解释器预装 |
| Embedding 模型 | 本地 ONNX 推理零外联，但 `all-MiniLM-L6-v2` 首次使用会从 HuggingFace 下载并缓存到 `~/.embabel/models/`——须预置该缓存目录，或经内网 HTTP 镜像供给（加载器支持 HTTP resolve + 本地缓存） |
| OfficeCli 等开源制品 | 按 10.8 节：内网制品库下发、SHA-256 校验、`autoUpdate false` 固定版本、出站默认拒绝 |
| RAG | Lucene（零外部依赖）或内网 pgvector/Neo4j 实例 |
| 远程 MCP | 配置中移除公网 Server（search.parallel.ai 类），仅保留内网端点与本地 STDIO 进程 |
| 构建与镜像 | 构建期 Maven 依赖走内网 Nexus；容器基础镜像走内网 registry |
...
| 技能加载与激活 | `embabel-agent-skills/.../skills/Skills.kt` |
| 技能脚本沙箱引擎 | `embabel-agent-skills/.../script/DockerSkillScriptExecutionEngine.kt` |
| MCP 工具组（懒加载与过滤） | `embabel-agent-api/.../tools/mcp/McpToolGroup.kt` |
| ToolCallContext 外传 MCP _meta | `embabel-agent-api/.../tools/mcp/ToolCallContextMcpMetaConverter.kt` |
